...
using BlogProjem.Services.Utilities;
...
namespace BlogProjem.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        /// <returns> Asenkron bir operasyon ile bizlere işlemin sonucunu DataResult tipinde döner.</returns>
        ...
        public async Task<IDataResult<CategoryDto>> UpdateAsync(CategoryUpdateDto categoryUpdateDto, string modifiedByName)
        {
            ...
            //Bizlerin eski kategoriye ihtiyacı olacak CreatedDate , Her bir güncellenmede CreatedDate ve CreatedByName alanı atanmayacak
            var oldCategory = await _unitOfWork.Categories.GetAsync(c => c.Id == categoryUpdateDto.Id); //Bizlere update için gelen kategoriyi çağırmış olacağız.
        ...
        {
            var result = await _unitOfWork.Categories.AnyAsync(c => c.Id == categoryId);        //Böyle bir kategori var mı? 
            if (result)
            {
                var category = await _unitOfWork.Categories.GetAsync(c => c.Id == categoryId);     //Kategoriyi getiriyoruz. Bizlerin bir tane kategorisi oldu. Bizlerin categoryUpdateDtoya map etmesi gerekiyor. Go to AutoMapper
                var categoryUpdateDto = _mapper.Map<CategoryUpdateDto>(category);        // Bu değerlerin map edilmesini sağlamış oluyoruz. Bu işlemler sonucunda elimizde bir tane categoryUpdateDto olmuşş oluyor.   
                return new DataResult<CategoryUpdateDto>(ResultStatus.Success, categoryUpdateDto);   //CategoryUpdateDto yu return etmiş oluyoruz . Controller sınıfımızda actionumuzda alırız.
            }
            else
            {
