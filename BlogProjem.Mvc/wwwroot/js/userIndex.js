                ...
                action: function (e, dt, node, config) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            dataTable.clear();                                  //Datatable içerisindeki tüm rowların temizlenmesini sağlayacak.(sıfırlıyoruz.)
                            ...
                            if (userListDto.ResultStatus === 0) {
                                $.each(userListDto.Users.$values,
                                  ...
                                  const newTableRow = dataTable.row.add([
                                        ...
                                        jqueryTableRow.attr('name', `${user.Id}`);  //Yeni eklenen tableRowun name özelliğine id eklemiş oluyoruz.
                                        /*Datatableın her bir değeri baştan yazmasını isteyeceğiz o yüzden her biri için .draw işlemi kullanmasına gerek yok.*/
                                    }); 
                                dataTable.draw();           /*Öncelikle  datatableı temizliyor . Sonrasında Datatableın foreach döngüsüyle dolmasını ve dolduktan sonra da tüm foreach döngüleri bittikten sonra tekrar ekrana çizilmesini gelmesini istiyoruz.*/
                                $('.spinner-border').hide();
    ...
    $(function () {
        ...
        placeHolderDiv.on('click',
            ...
            function (event) {
                ...
                const actionUrl = form.attr('action');               //Form üzerinden gidilecek actionun Url i 
                const dataToSend = new FormData(form.get(0));        //Yeni bir form verisi oluşturuyoruz. 0.İndeksteki formun bilgilerini buraya eklemiş oluyoruz.
                $.ajax({                                                                                
                    url: actionUrl,                                     //Bunlar nereye gidicek                                                                                                                
                    type: 'POST',                                       //POST İşlemi
                    data: dataToSend,                                   //Veri olarak ne gönderilecek ? 
                    processData: false,                                 
                    contentType: false,                                //Bu işlemi yaptığımızda artık bir upload işlemi gerçekleşiyorsa bununla ilgili hem JQuery Ajaxi hemde form ile ilgili verilerin nasıl alındıgı kısmını eklemiş oluyoruz.
                    success: function (data) {                                  
                        console.log(data);
                        ...
                        if (isValid) {
                         ...
                         `
                            ...
                            const jqueryTableRow = $(newTableRow);
                            jqueryTableRow.attr('name', `${userAddAjaxModel.UserDto.User.Id}`);  //Yeni eklenen tableRowun name özelliğine id eklemiş oluyoruz.
